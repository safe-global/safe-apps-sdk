...
const GetTransactionReceipt = ({ sdk }: OwnProps): React.ReactElement => {
  ...
  const handleClick = async () => {
    const response = await sdk.eth.getTransactionReceipt([hash])
  ...
  return (
    <Container>
      <Text>getTransactionReceipt(hash)</Text>
      <TextInput
        value={hash}
